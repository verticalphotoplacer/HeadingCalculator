...
def resolve_tool(name, systemid=0):
    """ Find full path of tool """
    
    # if system is win -> add win folder
    if systemid == 0:
        basepath = join(resource_path('tool'), 'win')
    # else, add linux folder
    else:
        basepath = join(resource_path('tool'), 'linux')
...
executable = resolve_tool("exiftool", platform_id)
...
class ExifTool(object):
    ...
    def start(self):
        ...
        with open(devnull, "w") as devn:
            self._process = subprocess.Popen(
                [self.executable, "-stay_open", "True",  "-@", "-",
                 "-common_args", "-G", "-n"],
                stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                stderr=devn, startupinfo=startupinfo)
        self.running = True
